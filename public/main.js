...
aboutUsBtn.addEventListener('click', () => {
    ...
    document.body.classList.add('modal-active'); // Додаємо фільтр для розмиття
});

// Закриття модального вікна "Про нас"
closeModalAboutUsBtn.addEventListener('click', () => {
    modalOverlayAboutUs.style.display = 'none';
    document.body.classList.remove('modal-active'); // Видаляємо фільтр
});
...
async function toggleTaskStatus(taskId, checkbox) {
    try {
        ...
        // Store completed state in localStorage
...
async function handleTaskSubmit(event) {
    ...
    try {
        ...
        const tasksContainer = document.getElementById('tasks-container');
        tasksContainer.innerHTML = ''; // Очищуємо контейнер

        // Отримуємо оновлений список завдань
        ...
        // Фільтруємо завдання для поточного дня
...
function displayTasksForDay(tasks) {
    ...
    if (tasks && tasks.length > 0) {
        tasks.forEach(task => {
            // Check both server and local storage state
            const serverCompleted = task.completedBy?.some(
                completion => completion.username === username
            );
            const localCompleted = completedTasks[task._id];
